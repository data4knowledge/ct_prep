...
class ActionCodeList(Action):
  ...
  def process(self, manifest):
    ...
    if (previous_dict == None) or (not previous_dict == None and DeepDiff(previous_dict, codelist, ignore_order=True)):
      ...
      uuid = str(uuid4())
      uri = "%sdataset/cdisc/ct/sc/%s/%s/%s" % (ServiceEnvironment().get("BASE_URI"), self.date, self.scheme, self.identifier)
      ...
      for cl in codelist['terms']:
        ...
        if diff:  
          uuid = str(uuid4())
          uri = "%sdataset/cdisc/ct/sc/%s/%s/%s/%s" % (ServiceEnvironment().get("BASE_URI"), self.date, self.scheme, self.identifier, cl['identifier'])
